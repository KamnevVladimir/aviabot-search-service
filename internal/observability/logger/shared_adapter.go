...
func (s *SharedAdapter) Info(event string, data map[string]interface{}) {
	_ = s.c.Info(event, "", data)
}
func (s *SharedAdapter) Error(event string, data map[string]interface{}) {
	_ = s.c.Error(nil, event, data)
}
